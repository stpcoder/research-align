...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(!assignment||assignment.status==='cancelled')return json({error:'Assignment not found'},404)
    ...
    const{data:sentRows,error:sentLookupError}=await admin.from('notifications').select('id,metadata,created_at').eq('assignment_id',assignmentId).eq('channel','email').eq('kind','schedule_confirmation').eq('status','sent').order('created_at',{ascending:false})
    if(sentLookupError)throw sentLookupError
    const snapshotStart=String(assignment.starts_at)
    const snapshotEnd=String(assignment.ends_at)
    const alreadySent=(sentRows||[]).find((row:any)=>row.metadata?.starts_at===snapshotStart&&row.metadata?.ends_at===snapshotEnd)
    if(alreadySent)return json({status:'sent',already_sent:true,notification_id:alreadySent.id})
    const event=(sentRows||[]).length?'changed':'confirmed'
    const destination=addressOf(response.contact_email||'')

    const{data:notification,error:notificationError}=await admin.from('notifications').insert({study_id:studyId,response_id:response.id,assignment_id:assignmentId,channel:'email',destination:destination||'email-unavailable',status:destination?'pending':'skipped',kind:'schedule_confirmation',metadata:{event,starts_at:snapshotStart,ends_at:snapshotEnd,session_label:assignment.session_label}}).select('id').single()
    if(notificationError)throw notificationError
    notificationId=notification.id
    if(!destination){await admin.from('notifications').update({error:'신청 이메일 없음'}).eq('id',notificationId);return json({status:'skipped',reason:'missing_email',notification_id:notificationId})}
