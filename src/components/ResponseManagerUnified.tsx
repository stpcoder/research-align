...
export default function ResponseManagerUnified({study}:{study:Study}){
  ...
  ]).then(([r,a,t])=>{const next=(r.data||[]) as ResponseRow[];setRows(next);setAssignments((a.data||[]) as Assignment[]);setThreads((t.data||[]) as ContactThread[]);setSelectedId(current=>{const preferred=participantFromUrl();const resolved=preferred&&next.some(row=>row.id===preferred)?preferred:current&&next.some(row=>row.id===current)?current:next[0]?.id||null;if(resolved)setParticipantInUrl(resolved);return resolved})})},[study.id])
  ...
  function selectParticipant(id:string){setSelectedId(id);setParticipantInUrl(id)}
