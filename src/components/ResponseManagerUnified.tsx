...
function assignmentStatus(value:string):AdminStatus{return value==='confirmed'?'confirmed':value==='draft'?'draft':value==='completed'?'completed':'neutral'}
...
export default function ResponseManagerUnified({study}:{study:Study}){
  ...
  function exportCsv(){const headers=['제출일시','이름','이메일','전화번호',...fields.map(f=>f.label),'배정 일정'];const lines=rows.map(row=>{const schedule=assignments.filter(a=>a.response_id===row.id).map(a=>`${a.session_label} ${dateTime(a.starts_at)} (${a.status})`).join(' | ');return[dateTime(row.submitted_at),responseName(study,row),row.contact_email||'',row.contact_phone||'',...fields.map(f=>answerText(f,row)),schedule].map(csvCell).join(',')});download(`${study.slug}-responses.csv`,'\ufeff'+[headers.map(csvCell).join(','),...lines].join('\n'),'text/csv;charset=utf-8')}
  ...
  const sidebar=<AdminSurface>
    <AdminPanelHeader title="신청자" meta={`${rows.length}명`}/>
    <input className="ar-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="이름 또는 연락처 검색"/>
    <div className="aui-list">{filtered.map(row=><AdminListItem key={row.id} active={selectedId===row.id} title={responseName(study,row)} subtitle={row.contact_email||row.contact_phone||'연락처 없음'} meta={`제출 ${dateTime(row.submitted_at)}`} onClick={()=>setSelectedId(row.id)}/>)}{!filtered.length&&<div className="empty compact">검색 결과가 없습니다.</div>}</div>
  </AdminSurface>

  return <div>
    <AdminPageHeader kicker="PARTICIPANTS" title="신청자" description="신청 내용, 가능한 시간, 배정 일정과 연락 기록을 참가자별로 확인합니다." actions={<><button className="btn secondary small" onClick={exportCsv}>CSV 내보내기</button><button className="btn ghost small" onClick={exportJson}>JSON 내보내기</button></>}/>
    <AdminSplitView sidebar={sidebar} sidebarWidth={310}>
      <AdminSurface>
        {!selected?<div className="empty">확인할 신청자를 선택해주세요.</div>:<>
          ...
          <div className="ar-section"><h4>일정 기록</h4>{selectedAssignments.map(a=><div className="ar-record" key={a.id}><div><b>{a.session_label}</b><span>{dateTime(a.starts_at)} – {dateTime(a.ends_at)}</span></div><StatusBadge status={assignmentStatus(a.status)} label={a.status==='confirmed'?'확정':a.status==='draft'?'미확정':a.status==='completed'?'완료':a.status}/></div>)}{!selectedAssignments.length&&<span className="muted small">아직 배정된 일정이 없습니다.</span>}</div>
          <div className="ar-section"><h4>연락 기록</h4>{selectedThreads.map(thread=><div className="ar-record" key={thread.id}><div><b>{thread.subject||'연락'}</b><span>{thread.channel.toUpperCase()} · 최근 {dateTime(thread.last_message_at)}</span></div><StatusBadge status="neutral" label={thread.status==='open'?'진행 중':'종료'}/></div>)}{!selectedThreads.length&&<span className="muted small">아직 연락 기록이 없습니다.</span>}</div>
        </>}
      </AdminSurface>
    </AdminSplitView>
  </div>
