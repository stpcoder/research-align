...
function ChoiceEditor({options,onChange}:{options:string[];onChange:(options:string[])=>void}){
  ...
  return<div className="af-options-editor"><div className="af-options-head"><div><strong>선택지</strong><span>각 선택지를 하나씩 추가하고 순서를 바꿀 수 있습니다.</span></div><button type="button" className="btn secondary small" onClick={add}>+ 선택지 추가</button></div><div className="af-options-list">{options.map((option,index)=><div className={`af-option-row ${dragIndex===index?'dragging':''}`} key={index} onDragOver={e=>e.preventDefault()} onDrop={()=>drop(index)}><button type="button" className="af-option-handle" draggable onDragStart={e=>{setDragIndex(index);e.dataTransfer.effectAllowed='move'}} onDragEnd={()=>setDragIndex(null)} aria-label={`${index+1}번 선택지 순서 변경`} title="드래그해서 순서 변경">⋮⋮</button><span className="af-option-number">{index+1}</span><input value={option} onChange={e=>update(index,e.target.value)} placeholder={`선택지 ${index+1}`}/><button type="button" className="af-option-remove" onClick={()=>remove(index)} aria-label="선택지 삭제">×</button></div>)}{!options.length&&<div className="af-options-empty">선택지가 없습니다. ‘선택지 추가’를 눌러 추가하세요.</div>}</div></div>
}
...
function AvailabilityEditor({field,onChange}:{field:FormField;onChange:(patch:Partial<FormField>)=>void}){
  ...
  return<div className="availability-editor af-availability">
    <section className="af-settings-section"><div className="af-settings-heading"><div><strong>세션 설정</strong><span>일정 화면과 참가자 안내 메일에 함께 사용됩니다.</span></div></div><div className="af-session-primary"><label className="aui-field af-session-name"><span>세션 이름</span><input value={field.sessionLabel||''} onChange={e=>onChange({sessionLabel:e.target.value})}/><small>예: 사전 교육, 본 실험</small></label><label className="aui-field af-compact-field"><span>소요 시간</span><div className="af-input-suffix"><input type="number" min={5} value={field.duration||60} onChange={e=>onChange({duration:+e.target.value})}/><span>분</span></div></label><label className="aui-field af-compact-field"><span>시작 간격</span><select value={field.stepMinutes||30} onChange={e=>onChange({stepMinutes:+e.target.value})}><option value={15}>15분</option><option value={30}>30분</option><option value={60}>60분</option></select></label></div><div className="af-session-ops"><label className="aui-field"><span>장소</span><input value={field.location||''} onChange={e=>onChange({location:e.target.value})} placeholder="예: C5 123호 / Zoom"/></label><label className="aui-field af-buffer"><span>준비 시간</span><select value={field.bufferMinutes||0} onChange={e=>onChange({bufferMinutes:+e.target.value})}><option value={0}>없음</option><option value={5}>5분</option><option value={10}>10분</option><option value={15}>15분</option><option value={30}>30분</option><option value={60}>60분</option></select><small>세션 뒤에 다음 일정을 잡지 않을 시간입니다.</small></label></div><label className="aui-field"><span>참가자 안내</span><textarea className="af-session-instructions" value={field.instructions||''} onChange={e=>onChange({instructions:e.target.value})} placeholder="예: 노트북을 지참해주세요. 건물 1층에서 연락주세요."/></label></section>

    <section className="af-settings-section"><div className="af-settings-heading"><div><strong>참가자 선택 규칙</strong><span>참가자가 몇 개의 시간을 고르고 선호 순위를 지정할지 설정합니다.</span></div></div><div className="af-rule-row"><label className="aui-field"><span>최소 선택</span><input type="number" min={1} value={field.min||1} onChange={e=>onChange({min:+e.target.value})}/></label><label className="aui-field"><span>최대 선택</span><input type="number" min={1} value={field.max||''} placeholder="제한 없음" onChange={e=>onChange({max:e.target.value?+e.target.value:null})}/></label><label className="aui-field"><span>선호 순위</span><input type="number" min={0} value={field.rankTop||0} onChange={e=>onChange({rankTop:+e.target.value})}/><small>0이면 사용하지 않음</small></label></div></section>

    <section className="af-settings-section"><div className="af-settings-heading blackout-head"><div><strong>날짜</strong><span>하루씩 추가하거나 7일·기간 전체를 한 번에 추가할 수 있습니다.</span></div>{dates.length>0&&<button type="button" className="btn ghost small" onClick={clearDates}>모두 지우기</button>}</div><div className="af-date-entry"><input type="date" value={newDate} onChange={e=>setNewDate(e.target.value)}/><button type="button" className="btn secondary" onClick={addDate}>날짜 추가</button></div><div className="af-range-entry"><label><span>시작일</span><input type="date" value={rangeStart} onChange={e=>{setRangeStart(e.target.value);if(rangeEnd&&rangeEnd<e.target.value)setRangeEnd('')}}/></label><label><span>종료일</span><input type="date" min={rangeStart||undefined} value={rangeEnd} onChange={e=>setRangeEnd(e.target.value)}/></label><div className="af-range-actions"><button type="button" className="btn secondary" onClick={addWeek} disabled={!rangeStart}>7일 추가</button><button type="button" className="btn secondary" onClick={()=>addRange()} disabled={!rangeStart||!rangeEnd}>기간 추가</button></div></div><div className="af-date-count">{dates.length>0?`${dates.length}일이 참가자에게 표시됩니다.`:'아직 날짜가 없습니다.'}</div><div className="selected-dates af-date-chips">{dates.map(date=><button type="button" key={date} className="date-chip" onClick={()=>removeDate(date)}>{dateLabel(date)} <span>×</span></button>)}</div></section>

    <section className="af-settings-section"><div className="af-settings-heading"><div><strong>운영 시간</strong><span>모든 날짜에 적용할 기본 시작·종료 시간을 지정합니다.</span></div></div><div className="af-hours-entry"><label className="aui-field"><span>시작</span><input type="time" value={start} onChange={e=>updateHours('start',e.target.value)}/></label><span className="af-time-separator">–</span><label className="aui-field"><span>종료</span><input type="time" value={end} onChange={e=>updateHours('end',e.target.value)}/></label></div></section>

    <section className="af-settings-section af-blackout-section"><div className="af-settings-heading blackout-head"><div><strong>제외할 시간</strong><span>회색으로 칠한 시간은 참가자에게 선택지로 표시되지 않습니다.</span></div>{(field.blockedSlots||[]).length>0&&<button type="button" className="btn ghost small" onClick={()=>onChange({blockedSlots:[]})}>모두 열기</button>}</div>{!dates.length?<div className="blackout-empty">먼저 날짜를 추가하면 타임테이블이 표시됩니다.</div>:<div className="blackout-scroll"><div className="blackout-grid" style={{gridTemplateColumns:`76px repeat(${dates.length}, minmax(112px,1fr))`}} onDragStart={e=>e.preventDefault()}><div className="blackout-corner">시간</div>{dates.map(d=><div className="blackout-date" key={d}>{dateLabel(d)}</div>)}{rows.flatMap(m=>[<div className="blackout-time" key={`t-${m}`}>{timeText(m)}</div>,...dates.map(date=>{const slot=`${date}T${timeText(m)}`;const off=blocked.has(slot);return<button type="button" key={slot} className={`blackout-cell ${off?'blocked':''}`} onPointerDown={e=>{e.preventDefault();begin(slot)}} onPointerEnter={()=>enter(slot)}>{off?'사용 안 함':'사용 가능'}</button>})])}</div></div>}<div className="blackout-legend"><span><i className="legend-allowed"/>사용 가능</span><span><i className="legend-blocked"/>사용 안 함</span><span className="muted">드래그해서 여러 칸을 한 번에 변경할 수 있습니다.</span></div></section>
  </div>
}
