...
export default function ScheduleUnified({study}:{study:Study}){
  ...
  function isFinal(a:Assignment|null){return !!a&&a.status!=='draft'&&a.status!=='cancelled'}
  ...
  function priorMissing(row:ResponseRow,field:FormField){const index=fields.findIndex(f=>f.id===field.id);if(index<=0)return null;for(const prior of fields.slice(0,index)){const a=assignmentFor(row,prior);if(!isFinal(a))return prior}return null}

  function participantProgress(row:ResponseRow){let final=0,legacyDraft=0;for(const field of fields){const a=assignmentFor(row,field);if(isFinal(a))final++;else if(a?.status==='draft')legacyDraft++}return{final,total:fields.length,legacyDraft}}
  const orderedPeople=useMemo(()=>[...responses].sort((a,b)=>{const ap=participantProgress(a),bp=participantProgress(b);const ar=ap.final===ap.total?2:ap.final>0?1:0,br=bp.final===bp.total?2:bp.final>0?1:0;return ar-br||participantName(a).localeCompare(participantName(b),'ko')}),[responses,assignments,fields])
  ...
  async function confirmLegacy(a:Assignment){setBusy(true);const{error}=await supabase.from('assignments').update({status:'confirmed'}).eq('id',a.id);setBusy(false);if(error)alert(error.message);else await load()}
  async function removeAssignment(a:Assignment){if(!confirm('현재 일정을 삭제할까요?'))return;setBusy(true);const{error}=await supabase.from('assignments').delete().eq('id',a.id);setBusy(false);if(error)alert(error.message);else await load()}

  if(!fields.length)return<div className="empty">신청서에 시간 선택 문항을 먼저 추가해주세요.</div>

  const sidebar=<AdminSurface className="ss-people"><AdminPanelHeader title="참가자" meta={`${responses.length}명`} description="사람을 선택한 뒤 오른쪽에서 세션과 시간을 정합니다."/><div className="aui-list">{orderedPeople.map(row=>{const progress=participantProgress(row);const done=progress.total>0&&progress.final===progress.total;const label=done?'모든 일정 확정':`${progress.final}/${progress.total} 확정${progress.legacyDraft?` · 확인 필요 ${progress.legacyDraft}`:''}`;return<AdminListItem key={row.id} active={selected?.id===row.id} title={participantName(row)} subtitle={row.contact_email||row.contact_phone||'연락처 없음'} status={<StatusBadge status={done?'confirmed':'unassigned'} label={label}/>} onClick={()=>setSelectedId(row.id)}/>})}</div></AdminSurface>

  return<div className="ss-root">
    <AdminPageHeader kicker="SCHEDULE" title="일정 정하기" description="사람을 고르고, 세션을 고르고, 시간을 선택한 다음 한 번만 확정합니다."/>
    <div className="ss-steps"><span className="done">1 참가자 선택</span><span>→</span><span className={selected?'done':''}>2 세션 선택</span><span>→</span><span className={pendingSlot?'done':''}>3 시간 선택</span><span>→</span><span>4 확정</span></div>
    <AdminSplitView sidebar={sidebar} sidebarWidth={300}>
      <div className="ss-main">
        <AdminSurface className="ss-sessions">
          <AdminPanelHeader title={selected?`${participantName(selected)}의 세션`:'세션'} description="세션을 바꾸면 해당 세션의 시간표가 아래에 표시됩니다."/>
          <div className="ss-session-list">{fields.map((field,index)=>{const a=selected?assignmentFor(selected,field):null;const state=isFinal(a)?'confirmed':a?.status==='draft'?'draft':'unassigned';return<button type="button" key={field.id} className={`ss-session ${activeField?.id===field.id?'active':''}`} onClick={()=>setActiveSessionKey(sessionKey(field))}><span className="ss-session-index">{index+1}</span><span className="ss-session-copy"><strong>{field.sessionLabel||field.label}</strong><small>{field.duration||60}분{a?` · ${fmtDateTime(a.starts_at)}`:''}</small></span><StatusBadge status={state} label={state==='confirmed'?'확정':state==='draft'?'확인 필요':'미배정'}/></button>})}</div>
        </AdminSurface>

        {selected&&activeField&&<AdminSurface className="ss-current">
          <div className="ss-current-copy"><small>{participantName(selected)} · {activeField.sessionLabel||activeField.label}</small>{currentAssignment?<><strong>{fmtDateTime(currentAssignment.starts_at)}</strong><span>{currentAssignment.status==='draft'?'이전 방식에서 저장된 확정 전 일정입니다.':'현재 확정된 일정입니다.'}</span></>:<><strong>아직 확정된 일정이 없습니다.</strong><span>{missingPrior?`${missingPrior.sessionLabel||missingPrior.label}을 먼저 확정해주세요.`:'아래에서 참가자가 선택한 시간 하나를 고르세요.'}</span></>}</div><div className="ss-current-actions">{currentAssignment?.status==='draft'&&<button className="btn" disabled={busy} onClick={()=>confirmLegacy(currentAssignment)}>현재 시간 확정</button>}{currentAssignment&&<button className="btn ghost" disabled={busy} onClick={()=>removeAssignment(currentAssignment)}>일정 삭제</button>}</div>
        </AdminSurface>}

        {selected&&activeField&&<AdminSurface className="ss-grid-panel">
          <div className="ss-grid-head"><div><h3>{activeField.sessionLabel||activeField.label} 시간표</h3><p>색보다 문구를 기준으로 보세요. 다른 사람이 원한 시간은 작은 보조 문구로만 표시되고, 실제 예약은 ‘예약됨’으로 표시됩니다.</p></div><div className="ss-legend"><span><i className="available"/>선택 가능</span><span><i className="preferred"/>선호 시간</span><span><i className="chosen"/>방금 선택</span><span><i className="current"/>현재 확정</span><span><i className="occupied"/>다른 일정</span></div></div>
