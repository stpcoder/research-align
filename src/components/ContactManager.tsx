...
export default function ContactManager({study}:{study:Study}){
  ...
  return<div className="contact-workspace"><div className="contact-header"><h2>연락</h2><div className="contact-mailbox">{identityEmail?<><span className="contact-mailbox-address">{identityEmail}</span>{lastSyncedAt&&<span className="muted small">최근 확인 {fmt(lastSyncedAt)}</span>}<button className="btn secondary small" onClick={()=>syncMail(true)} disabled={syncing}>{syncing?'확인 중…':'새 메일 확인'}</button></>:<button className="btn secondary small" onClick={connectResearchEmail} disabled={busy}>{busy?'연결 중…':'연구용 이메일 연결'}</button>}</div></div>{notice&&<div className="notice">{notice}</div>}
    <div className="contact-browser-new"><aside className="card contact-participants"><input className="contact-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="이름 또는 이메일 검색"/>
      <div className="contact-list-section"><div className="section-head"><h3>문의 대기</h3><span className={`pill ${pendingCount?'contact-pending-pill':''}`}>{pendingCount}</span></div><div className="people contact-people">{visibleInquiries.map(thread=><button type="button" key={thread.id} className={`person contact-person ${selection===`thread:${thread.id}`?'active':''}`} onClick={()=>selectInquiry(thread.id)}><span className="contact-person-title"><strong>{thread.requester_name||thread.participant_address}</strong>{thread.status==='pending'&&<span className="pill contact-pending-pill">답변 필요</span>}</span><span className="muted small">{thread.participant_address}</span>{thread.last_message_at&&<span className="contact-person-meta"><span>{fmt(thread.last_message_at)}</span></span>}</button>)}{!visibleInquiries.length&&<div className="contact-list-empty">대기 문의 없음</div>}</div></div>
      <div className="contact-list-divider"/><div className="contact-list-section"><div className="section-head"><h3>참가자</h3><span className="muted small">{visibleResponses.length}/{responses.length}명</span></div><div className="people contact-people">{visibleResponses.map(row=>{const thread=threadForResponse(row);return<button type="button" key={row.id} className={`person contact-person ${selection===`response:${row.id}`?'active':''}`} onClick={()=>selectResponse(row.id)}><span className="contact-person-title"><strong>{participantName(row)}</strong>{thread?.status==='pending'&&<span className="pill contact-pending-pill">답변 필요</span>}</span><span className="muted small">{thread?.participant_address||row.contact_email||'이메일 없음'}</span>{thread?.last_message_at&&<span className="contact-person-meta"><span>{fmt(thread.last_message_at)}</span></span>}</button>})}{!visibleResponses.length&&<div className="empty compact">검색 결과가 없습니다.</div>}</div></div>
    </aside>
    <section className="card conversation-panel">{!selectedResponse&&!selectedThread?<div className="empty">문의자 또는 참가자를 선택하세요.</div>:<><div className="conversation-head"><div><div className="contact-conversation-title"><h2>{selectedName}</h2>{selectedThread?.status==='pending'&&<span className="pill contact-pending-pill">답변 필요</span>}</div><div className="response-meta">{selectedEmail&&<span>{selectedEmail}</span>}{selectedResponse?.contact_phone&&<span>{selectedResponse.contact_phone}</span>}{selectedThread?.source==='public_inquiry'&&<span>{selectedThread.response_id?'신청자 문의':'신청 전 문의'}</span>}</div></div><div className="row">{selectedResponse&&<><button type="button" className="btn secondary small" onClick={()=>navigateResearcher('schedule',selectedResponse.id)}>일정 보기</button><button type="button" className="btn ghost small" onClick={()=>navigateResearcher('responses',selectedResponse.id)}>신청 내용</button></>}{selectedThread?.source==='public_inquiry'&&<button className="btn ghost small" onClick={closeInquiry}>처리 완료</button>}</div></div>
