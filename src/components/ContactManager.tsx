...
export default function ContactManager({study}:{study:Study}){
  ...
  const pageActions=<AdminActions>{identityEmail?<><span className="muted small">{identityEmail}{lastSyncedAt?` · ${fmt(lastSyncedAt)}`:''}</span><AdminButton variant="secondary" size="sm" onClick={()=>syncMail(true)} disabled={syncing}>{syncing?'확인 중…':'새 메일 확인'}</AdminButton></>:<AdminButton variant="secondary" size="sm" onClick={connectResearchEmail} disabled={busy}>{busy?'연결 중…':'연구용 이메일 연결'}</AdminButton>}</AdminActions>
  const sidebar=<AdminSurface className="contact-participants"><AdminInput className="contact-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="이름 또는 이메일 검색"/><AdminPanelHeader title="문의" meta={String(pendingCount)}/><div className="aui-list">{visibleInquiries.map(thread=><AdminListItem key={thread.id} active={selection===`thread:${thread.id}`} title={thread.requester_name||thread.participant_address} subtitle={thread.participant_address} meta={thread.last_message_at?fmt(thread.last_message_at):null} status={thread.status==='pending'?<StatusBadge status="danger" label="답변 필요"/>:undefined} onClick={()=>selectInquiry(thread.id)}/>)}{!visibleInquiries.length&&<div className="empty compact">대기 문의 없음</div>}</div><AdminDivider className="contact-list-divider"/><AdminPanelHeader title="참가자" meta={`${visibleResponses.length}/${responses.length}`}/><div className="aui-list">{visibleResponses.map(row=>{const thread=threadForResponse(row);return<AdminListItem key={row.id} active={selection===`response:${row.id}`} title={participantName(row)} subtitle={thread?.participant_address||row.contact_email||'이메일 없음'} meta={thread?.last_message_at?fmt(thread.last_message_at):null} status={thread?.status==='pending'?<StatusBadge status="danger" label="답변 필요"/>:undefined} onClick={()=>selectResponse(row.id)}/>})}{!visibleResponses.length&&<div className="empty compact">검색 결과 없음</div>}</div></AdminSurface>

  return<div className="contact-workspace"><AdminPageHeader title="연락" actions={pageActions}/>{notice&&<div className="notice">{notice}</div>}<AdminSplitView sidebar={sidebar} sidebarWidth={290}><AdminSurface className="conversation-panel">{!selectedResponse&&!selectedThread?<div className="empty">문의자 또는 참가자를 선택하세요.</div>:<><div className="conversation-head"><div><div className="contact-conversation-title"><h2>{selectedName}</h2>{selectedThread?.status==='pending'&&<StatusBadge status="danger" label="답변 필요"/>}</div><div className="response-meta">{selectedEmail&&<span>{selectedEmail}</span>}{selectedResponse?.contact_phone&&<span>{selectedResponse.contact_phone}</span>}{selectedThread?.source==='public_inquiry'&&<span>{selectedThread.response_id?'신청자 문의':'신청 전 문의'}</span>}</div></div><AdminActions>{selectedResponse&&<><AdminButton variant="secondary" size="sm" onClick={()=>navigateResearcher('schedule',selectedResponse.id)}>일정 보기</AdminButton><AdminButton variant="ghost" size="sm" onClick={()=>navigateResearcher('responses',selectedResponse.id)}>신청 내용</AdminButton></>}{selectedThread?.source==='public_inquiry'&&<AdminButton variant="ghost" size="sm" onClick={closeInquiry}>처리 완료</AdminButton>}</AdminActions></div>
      {selectedResponse&&availabilityFields.length>0&&<div className="contact-schedule-context"><strong className="contact-schedule-title">일정</strong><AdminDataList>{availabilityFields.map(field=>{const assignment=assignmentFor(selectedResponse,field);return<AdminDataRow key={field.id} title={field.sessionLabel||field.label} detail={assignment?fmt(assignment.starts_at):availabilityPreview(selectedResponse,field)} trailing={assignment?<StatusBadge status={assignmentBadge(assignment.status)} label={assignmentLabel(assignment.status)}/>:<StatusBadge status="unassigned" label="미정"/>}/>} )}</AdminDataList></div>}
      <div className="conversation-messages">{messages.map(message=><div key={message.id} className={`conversation-message ${message.direction}`}><div>{message.body}</div>{message.metadata?.source==='schedule_notification'&&<span className="contact-auto-label">자동 일정 안내</span>}<span>{message.direction==='outbound'?'보냄':'받음'} · {fmt(message.sent_at)}</span></div>)}{!messages.length&&<div className="conversation-empty">아직 대화가 없습니다.</div>}</div>
      <div className="composer">{!selectedThread&&selectedResponse&&<AdminField label="제목"><AdminInput value={subject} onChange={e=>setSubject(e.target.value)} placeholder="예: 실험 일정 안내"/></AdminField>}<AdminField label="메시지"><AdminTextarea value={body} onChange={e=>setBody(e.target.value)} placeholder={`${selectedName||'문의자'}님에게 보낼 내용을 입력하세요.`}/></AdminField><div className="composer-footer contact-composer-actions"><AdminButton onClick={send} disabled={busy||!body.trim()||!selectedEmail||!identityEmail}>{busy?'보내는 중…':'이메일 보내기'}</AdminButton></div></div>
