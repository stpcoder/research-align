...
export default function ParticipantForm({slug}:{slug:string}){
  ...
  useEffect(()=>{supabase.from('studies').select('*').eq('slug',slug).eq('status','published').single().then(({data})=>{setStudy(data as Study||null);setLoading(false)})},[slug])
  function toggle(f:FormField,slot:string){setAvailability(v=>{const list=v[f.id]||[];const next=list.includes(slot)?list.filter(x=>x!==slot):[...list,slot];if(f.max&&next.length>f.max)return v;setPreferences(p=>{const current={...(p[f.id]||{})};Object.keys(current).forEach(k=>{if(!next.includes(current[k]))delete current[k]});return {...p,[f.id]:current}});return {...v,[f.id]:next}})}
  async function submit(e:FormEvent){
    e.preventDefault();if(!study)return;setError('')
    for(const f of study.form_config.fields){
      const a=answers[f.id]
      if(f.type==='availability'){
        const n=(availability[f.id]||[]).length
  ...
  return <div className="shell"><header className="topbar"><b className="brand">StudyForm</b><span className="muted small">참가 신청</span></header><main className="container narrow"><div className="participant-head"><span className="pill live">모집 중</span><h1>{study.title}</h1><p className="muted">{study.description}</p></div><form className="stack" onSubmit={submit}>{study.form_config.fields.map(f=><Question key={f.id} f={f} answers={answers} setAnswers={setAnswers} availability={availability} toggle={toggle} preferences={preferences} setPreferences={setPreferences}/>) }{error&&<div className="notice">{error}</div>}<div className="card stack"><div><b>입력한 내용을 확인해주세요.</b><p className="muted small">선택한 시간과 연락처를 확인한 뒤 제출해주세요. 연구자가 확인 후 일정을 안내해드립니다.</p></div><button className="btn" style={{width:'100%'}}>제출하기</button></div></form></main></div>
}

function Question({f,answers,setAnswers,availability,toggle,preferences,setPreferences}:{f:FormField,answers:Record<string,any>,setAnswers:any,availability:Record<string,string[]>,toggle:(f:FormField,s:string)=>void,preferences:Record<string,Record<string,string>>,setPreferences:any}){
  ...
  if(f.type==='availability'){
    ...
    return <div className="card question availability"><div><div className="question-title">{f.label}{f.required&&<span className="required"> *</span>}</div><div className="muted small">{f.sessionLabel||'실험'} · {f.duration}분 · 최소 {f.min||1}개{f.max?` / 최대 ${f.max}개`:''}</div></div><div className="availability-summary"><span><b>{selected.length}</b>개 선택</span><span className="muted small">날짜별로 참여할 수 있는 시간을 선택해주세요.</span></div><div className="day-columns">{group(slots).map(([day,list])=><div className="day" key={day}><div className="day-head">{new Date(`${day}T00:00:00`).toLocaleDateString('ko-KR',{weekday:'short',month:'short',day:'numeric'})}<span>{day}</span></div><div className="slot-list">{list.map(s=><button type="button" key={s} className={`slot-button ${selected.includes(s)?'selected':''}`} onClick={()=>toggle(f,s)}>{s.split('T')[1]}</button>)}</div></div>)}</div>{!slots.length&&<div className="empty compact">현재 선택할 수 있는 시간이 없습니다.</div>}{rank>0&&selected.length>0&&<div><b>선호 시간</b><p className="muted small">가장 선호하는 시간부터 순서대로 선택해주세요.</p><div className="ranking">{Array.from({length:Math.min(rank,selected.length)},(_,i)=><label key={i}>{i+1}순위<select value={preferences[f.id]?.[String(i+1)]||''} onChange={e=>setPreferences((p:any)=>({...p,[f.id]:{...(p[f.id]||{}),[String(i+1)]:e.target.value}}))}><option value="">선택</option>{selected.map(s=><option value={s} key={s}>{s.replace('T',' ')}</option>)}</select></label>)}</div></div>}</div>
  }
